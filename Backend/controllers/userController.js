...
export const stats = async (req, res) => {
  try {
    ...
    const offlineUsersArr = await Session.find({ status: 'offline' }).distinct('userId');
...
export const alerts = async (req, res) => {
  try {
    ...
    const lateJoin = await Session.find({ loginTime: { $gte: todayStart }, status: { $in: ['online','offline','disconnected'] } }).populate('userId', 'name employeeId').then(arr => arr.filter(s => {
      const hour = s.loginTime.getHours() + s.loginTime.getMinutes()/60;
      return hour >= 9.5; // 9:30 AM or later
    }).map(s => ({ sessionId: s._id, user: s.userId ? { id: s.userId._id, name: s.userId.name, employeeId: s.userId.employeeId } : null, loginTime: s.loginTime })));

    // extended shift: sessions whose totalDuration > 9 hours (32400s)
    const extended = await Session.find({ totalDuration: { $gt: 32400 } }).populate('userId', 'name employeeId').limit(50).sort({ createdAt: -1 });
    const extendedShift = extended.map(s => ({ sessionId: s._id, user: s.userId ? { id: s.userId._id, name: s.userId.name, employeeId: s.userId.employeeId } : null, totalDuration: s.totalDuration }));

    // unexpected disconnect: sessions with status disconnected in last 24h
    const since = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const disconnects = await Session.find({ status: 'disconnected', createdAt: { $gte: since } }).populate('userId','name employeeId').limit(50).sort({ createdAt: -1 });
...
export const start = async (req, res) => {
  try {
    ...
    // Prevent double-login: if there is an active online session, return it
    let existing = await Session.findOne({ userId: user._id, status: 'online' }).sort({ createdAt: -1 });
    if (existing) {
      ...
      user.isActive = true;
      await user.save();
      return res.json({ session: existing, message: 'existing' });
    }
    ...
    try {
      if (location) {
        ...
        if (typeof location === 'object' && location.lat && location.lng) { lat = location.lat; lng = location.lng; }
...
export const endBeacon = async (req, res) => {
  try {
    ...
    if (sessionId) {
      ...
      session = await Session.findOne({ userId: user._id, status: 'online' }).sort({ createdAt: -1 });
    }

    if (!session) return res.status(400).json({ message: 'No active session' });

    session.logoutTime = new Date();
    session.totalDuration = Math.max(0, (session.logoutTime - session.loginTime) / 1000);
    session.status = 'offline';
    await session.save();
    ...
    if (user) {
      ...
      if (u) { u.isActive = false; await u.save(); }
    }
...
export const logs = async (req, res) => {
  try {
    const { page = 1, limit = 100, from, to, userId, status, employeeId } = req.query || {};
...
export const exportLogs = async (req, res) => {
  try {
    const { from, to, userId, status, employeeId, limit = 10000 } = req.query || {};
    const filter = {};
    if (from) filter.createdAt = { ...(filter.createdAt || {}), $gte: new Date(from) };
    if (to) filter.createdAt = { ...(filter.createdAt || {}), $lte: new Date(to) };
    if (userId) filter.userId = userId;
    ...
    const sessions = await Session.find(filter).populate('userId', 'name employeeId').sort({ createdAt: -1 }).limit(lim);
    ...
    const header = ['sessionId','name','employeeId','loginTime','logoutTime','totalDuration','status','device','location','locationName','ip'];
...
export const cleanup = async (req, res) => {
  try {
    ...
    const result = await Session.deleteMany({ userId: null, createdAt: { $lt: cutoff } });
