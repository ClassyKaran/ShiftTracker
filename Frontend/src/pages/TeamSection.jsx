...
export default function TeamSection() {
    ...
    try {
      ...
      try {
        ...
        const merged = lists.map(l => ({
          teamlead: l.teamlead,
          tracked: (l.tracked || []).map(u => ({ ...u, ...(activeMap.get(String(u._id)) || {}) }))
        }));
        setTeamlists(merged);
  ...
  const renderRight = () => {
    if (!selected) return <div className="text-muted">Select a team lead to see their tracked employees</div>;
    if (loading) return <div className="text-muted">Loading team members…</div>;
    return (
      <div>
        <div className="mb-3">Viewing <strong>{selected.teamlead.name}</strong> ({selected.teamlead.employeeId}) — {selected.tracked.length} tracked</div>
            ...
            <tbody>
              {selected.tracked.map(u => (
                <tr key={u._id}>
                  ...
                  <td>{typeof u.totalDuration !== 'undefined' && u.totalDuration !== null ? `${Math.floor(u.totalDuration/3600)}h ${Math.floor((u.totalDuration%3600)/60)}m ${u.totalDuration%60}s` : (u.loginTime ? <span>{Math.floor((Date.now() - new Date(u.loginTime))/1000/60)}m</span> : '-')}</td>
              ...
              {selected.tracked.length === 0 && (
                <tr><td colSpan={8} className="text-center text-muted">No employees tracked by this teamlead</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="col-md-3">
          <div className="card mb-3">
            <div className="card-body">
              ...
              {teamlists.length === 0 && !loading && <div className="text-muted">No team leads found</div>}
              {teamlists.map((l) => (
                <div key={l.teamlead._id} className={`p-2 rounded mb-2 ${selected && String(selected.teamlead._id) === String(l.teamlead._id) ? 'bg-primary text-white' : 'bg-light'}`} style={{cursor:'pointer'}} onClick={() => setSelected(l)}>
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      <div className="fw-bold">{l.teamlead.name || '—'}</div>
                      <div className="small text-muted">{l.teamlead.employeeId || ''}</div>
                    </div>
                    <div className="badge bg-secondary rounded-pill">{(l.tracked || []).length}</div>
                  </div>
                </div>
              ))}
            </div>
